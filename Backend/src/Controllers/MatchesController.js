...
export default class TagsController {
    static async getAllUserMatches(req, res) {
        ...
        if (!matchesOne) return res.status(500).json({ msg: StatusMessage.INTERNAL_SERVER_ERROR })
        ...
        if (!matchesTwo) return res.status(500).json({ msg: StatusMessage.INTERNAL_SERVER_ERROR })
